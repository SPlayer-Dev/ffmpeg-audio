...
async function main() {
	...
	if (existsSync(slimDst)) {
		console.log(`清理旧的 ${slimDst}/ ...`);
		Deno.removeSync(slimDst, { recursive: true });
	}
	ensureDirSync(slimDst);
	...
	if (existsSync(configsDir)) {
		for (const entry of Deno.readDirSync(configsDir)) {
			if (!entry.isDirectory) continue;
			const logPath = join(configsDir, entry.name, "make_dryrun.log");
	...
	function copyCFile(relStr: string): string | null {
		const raw = join(ffmpegSrc, relStr);
		const resolved = normalize(raw);

		let relToSrc = "";
		if (resolved.startsWith(normalize(ffmpegSrc))) {
			relToSrc = relative(normalize(ffmpegSrc), resolved);
		} else {
			relToSrc = normalize(relStr);
		}

		const dst = join(slimDst, relToSrc);
	...
	for (const relStr of Array.from(allCFiles).sort()) {
		...
		if (dst) {
			...
			console.log(`  !! 缺失: ${join(ffmpegSrc, relStr)}`);
			cMiss++;
		}
	}
	...
	while (true) {
		...
		for (const entry of walkSync(slimDst, { exts: [".c"] })) {
			try {
				const content = Deno.readTextFileSync(entry.path);
				for (const match of content.matchAll(includeCPattern)) {
					const included = match[1];
					const candidate = normalize(join(dirname(entry.path), included));

					try {
						const relToSlimDst = relative(normalize(slimDst), candidate);
						const srcCandidate = join(ffmpegSrc, relToSlimDst);
						const dstCandidate = join(slimDst, relToSlimDst);

						if (!existsSync(dstCandidate) && existsSync(srcCandidate)) {
		...
		for (const relStr of Array.from(newlyNeeded).sort()) {
			...
			if (dst) {
				...
				console.log(
					`  !! 被 include 的 C 文件缺失: ${join(ffmpegSrc, relStr)}`,
				);
			}
		}
	}
	...
	for (const rootRel of Array.from(hScanRoots).sort()) {
		const srcDir = rootRel ? join(ffmpegSrc, rootRel) : ffmpegSrc;
		if (!existsSync(srcDir) || scanned.has(rootRel)) continue;
		scanned.add(rootRel);

		for (const entry of walkSync(srcDir, { exts: [".h"] })) {
			try {
				const relToSrc = relative(normalize(ffmpegSrc), normalize(entry.path));
				const dst = join(slimDst, relToSrc);
				copyFileSafe(entry.path, dst);
				hOk++;
			} catch (_e) {
				// 忽略异常文件
			}
	...
	for (const entry of walkSync(slimDst, { includeDirs: false })) {
		totalSlimFiles++;
		totalSlimBytes += Deno.statSync(entry.path).size;
	}
	console.log(`\nffmpeg_slim/ 生成完毕`);
	...
	async function createZip(sourceDir: string, destZipPath: string) {
		ensureDirSync(dirname(destZipPath));
		console.log(`\n打包 ${destZipPath} ...`);
		const zip = new JSZip();

		for (const entry of walkSync(sourceDir, { includeDirs: false })) {
			const arcname = toPosix(relative(sourceDir, entry.path));
			const fileData = Deno.readFileSync(entry.path);
			zip.file(arcname, fileData);
		}
		...
		Deno.writeFileSync(destZipPath, zipData);
	...
	ensureDirSync(join("crates", "ffmpeg_audio_sys", "vendor"));

	await createZip(
		slimDst,
		join("crates", "ffmpeg_audio_sys", "vendor", "ffmpeg_slim.zip"),
	);
	await createZip(
		configsDir,
		join("crates", "ffmpeg_audio_sys", "vendor", "configs.zip"),
	);
